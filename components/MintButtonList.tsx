...
export function MintButtonList({
    umi,
    guardList,
    candyMachine,
    candyGuard,
    ...
    const listItems = buttonGuardList.map((buttonGuard, index) => (
        <div>
            ...
            {buttonGuard.startTime > createBigInt(0) &&
                ...
                <>
                    <div className="text-center text-white font-semibold">Starting in</div>
                    <Timer
            ...
            <div className={`flex ${process.env.NEXT_PUBLIC_MULTIMINT && buttonGuard.allowed ? "justify-between" : "justify-center"}`}>
                <div className="text-center font-bold text-white text-base my-auto">{buttonGuard.mintText}</div>
                {
            ...
            <Tooltip label={buttonGuard.tooltip} aria-label="Mint button">
                <button
                    ...
                    className={`${!buttonGuard.allowed && "cursor-not-allowed opacity-40"} hover:opacity-50 bg-gold text-white text-center p-2 mt-2 w-full`}>Mint</button>
            </Tooltip>
        </div>
    ));
